...
test("wires chart protection picking, trailing stops and risk sizing into the workbench", async () => {
  ...
  assert.match(workbench, /const riskBalance = liveMode && currentLiveExecutionAccount/);
  assert.match(workbench, /currentLiveExecutionAccount\.riskCapital \+ liveAccountRealizedPnl/);
  assert.match(workbench, /: tradingMode === "capital"/);
  ...
  assert.match(workbench, /draftTriggerPrice: orderType === "market" \? undefined : Number\(orderTriggerPrice\)/);
  assert.match(workbench, /draftTriggerOrderType: orderType === "market" \? undefined : orderType/);
  assert.match(workbench, /type=\{hideTaskPrice \? "password" : "number"\}/);
  assert.match(workbench, /placeholder=\{hideTaskPrice \? "•••"/);
  assert.match(workbench, /const effectiveOpeningGapUnit: OpeningGapUnit = hideTaskPrice \? "percent" : openingGapUnit/);
  assert.match(workbench, /openingGapUnit: effectiveOpeningGapUnit/);
  assert.match(workbench, /<option value="price" disabled=\{hideTaskPrice\}>/);
  ...
  assert.match(chart, /data\.kind === "entry-trigger" \? "#69c0ff"/);
