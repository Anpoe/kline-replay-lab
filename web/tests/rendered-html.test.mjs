...
test("ships the K-line training workbench instead of the starter", async () => {
  ...
  assert.match(settingsPanel, /交易纪律/);
  assert.match(settingsPanel, /strictModeEnabled/);
  assert.match(settingsPanel, /requirePretradePlan/);
  assert.match(settingsPanel, /sopCheckEnabled/);
  assert.match(settingsPanel, /规划卡必填项/);
  assert.match(settingsPanel, /只影响事前规划卡/);
  assert.match(settingsPanel, /计划说明/);
  assert.match(settingsPanel, /discipline-switch/);
  assert.match(settingsPanel, /className=\{`discipline-switch/);
  assert.match(settingsPanel, /<span aria-hidden="true" \/>/);
  assert.match(settingsPanel, /aria-pressed=\{draft\.strictModeEnabled\}/);
  assert.match(settingsPanel, /aria-label=\{draft\.strictModeEnabled \? "关闭严格模式" : "开启严格模式"\}/);
  assert.match(workbench, /<SopPage/);
  assert.match(workbench, /sop_gate_blocked/);
  ...
  assert.match(snapshotsRoute, /aggregateTimeframeViewCandles/);
  assert.match(snapshotsRoute, /coversTimeframeViewWindow/);
